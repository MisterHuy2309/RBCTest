...
int Home_Encoder_0h = 1283, Home_Encoder_4h = 2389, Home_Encoder_8h = 2428;
...
void robotGamePadControl(vu8 speedMin, vu8 speedMid , vu8 speedMax)
{
	int daoGoc = 1800;
	int tocdo = 20;
	float tocdoXoay = 0;
	...
	if (UP && !DOWN && !RIGHT && !LEFT)
		robotRun(0 - daoGoc, tocdo);
	else if (!UP && DOWN && !RIGHT && !LEFT)
		robotRun(1800 - daoGoc, tocdo);
	else if (!UP && !DOWN && RIGHT && !LEFT)
		robotRun(900 - daoGoc, tocdo);
	else if (!UP && !DOWN && !RIGHT && LEFT)
		robotRun(-900 - daoGoc, tocdo);

	else if (UP && !DOWN && RIGHT && !LEFT)
		robotRun(450 - daoGoc, tocdo);
	else if (UP && !DOWN && !RIGHT && LEFT)
		robotRun(-450 - daoGoc, tocdo);
	else if (!UP && DOWN && RIGHT && !LEFT)
		robotRun(1350 - daoGoc, tocdo);
	else if (!UP && DOWN && !RIGHT && LEFT)
		robotRun(-1350 - daoGoc, tocdo);
//	else if	(!lJoy_LR) {
...
//	else if	(JTA < 15000)	robotRun(JTA, tocdo);
	...
	//if (!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR && lJoy_LR && robotIsRun()) robotStop(35);
	if (!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR && robotIsRun()) robotStop(35);
	//-------------- Xoay ----------------
//		if (rJoy_LR)
//			robotRotateStop();
//		else if (RJOY_LR < 50) 
...
vu8 Home_wheel_0h(void)
{
	...
	if(Home_0h == 1 && SS_RL == 0)																			{	mor_rotate_0h = 50; TIM2->CNT = 0; num_over_t2 = 0;return 0;}
...
vu8 Home_wheel_4h(void)
{
	...
	if(Home_4h == 1 && SS_RR == 0)																				{	mor_rotate_4h = 50; TIM4->CNT = 0; num_over_t4 = 0;return 0;}
...
vu8 Home_wheel_8h(void)
{
	...
	if(Home_8h == 1 && SS_FL == 0)																			{	mor_rotate_8h = 50; TIM3->CNT = 0; num_over_t3 = 0;return 0;}
