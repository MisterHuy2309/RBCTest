...
//        else 
			 if (TAM_GIAC) START_CELL_1();
				if(X) RESET_ENCODER(2);
			
			if(O) {PWM_Down_Up +=10; vTaskDelay(2000);}
			else if(VUONG) {PWM_Down_Up -=10; vTaskDelay(2000);}
			
//			if(VUONG) Robot_Task1();
//			else if (TAM_GIAC) START_CELL_1();
//			else if (O) START_CELL_2();
//			else if (X) START_CELL_3();
//			if(OPTIONS) Reset_ALL();
			
//			if(O) testbit = 1;
//			if(testbit == 1 ) Testtudong();
			else quet_tay_game();
       ////////////////vi du ve ham vua chay vua xoay  
//				if (TOI){//
//              MAIN_CONTROL(0,0,20,1,0);//ch?y va xoay >0 xoay trai, chay va xoay<0 xoay phai
//						MAIN_CONTROL(nho_IMU, 0, toc_p/5, gt*2, 20);
//							while(1){
//		
//								if(LUI)break;
...
//				}	
//            
//				else	MAIN_CONTROL(0,0,0,2,0); quet_tay_game();
