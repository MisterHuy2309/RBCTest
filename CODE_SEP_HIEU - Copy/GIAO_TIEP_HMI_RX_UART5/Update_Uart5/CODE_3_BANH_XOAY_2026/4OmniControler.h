...
int _robotIMUAngle=0,_IMUAngle_Before=0, _robotDelayGiamToc = 15; 
...
{
	if(T == 0) 			return 0;
	else 	if(T < 0) return (T*0.2 - 0.5)/_robotCurrentSpeed; // tuy thuoc robot ma co the he so toi thieu la 0.5, 1, 2...
				else 			return (T*0.2 + 0.5)/_robotCurrentSpeed;
}
//------------------------------------------------------------------------------
...
void robotRunAngle(float angle, int maxSpeed, float robotAngle)
{
    if(_robotRunAngle != angle || _robotRunSpeed != maxSpeed || _robotRotateAngle != robotAngle)
    {
			...
			robotRotate(robotAngle, (abs(robotAngle - _robotIMUAngle) > 100? (robotAngle > _robotIMUAngle ? 0.5: -0.5):(robotAngle - _robotIMUAngle)*0.005), 0);
		}
}
//------------------------------------------------------------------------------
