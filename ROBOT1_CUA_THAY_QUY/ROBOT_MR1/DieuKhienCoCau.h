...
int Lucbanbandau = 0;
///Laylaze
int lay_khoang_cach_a ;
int lay_khoang_cach_b ;
int KhoangCachA ;
int KhoangCachB ;
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	int data[]={0};
...
void ADCValue_Control(void)
{
	...
	{
		bientroBanBongValue = bientronangbong_SUM / 50;
...
//		while (abs(BienTroXoayValue - target_BT_Xoay) > 5) {vTaskDelay(1);};
		state_nang = 1, state_ha = 0, taget_BT_Nong_Ban = 358 + Troi_Bientro , lock_nang = 0 , XI_LANH = 254, XI_LANH_ON;
	}
	else if(R1 && X){
	target_BT_Xoay = 386;
	while (abs(BienTroXoayValue - target_BT_Xoay) > 2) {if(wantExit()) break; vTaskDelay(1);};
	taget_BT_Nong_Ban = 955 + Troi_Bientro , lock_nang = 0, XI_LANH = 0, XI_LANH_OFF;
	}
	else if (taget_BT_Nong_Ban >=945  && state_ha == 0 && BienTroNongBanValue > 945 ){
	state_nang = 0;  
	state_ha = 1;
	}
}
...
//int target_BT_BanBong = 0;
//int Min_BT_BanBong = 100;
//int Max_BT_BanBong = 100;

void Control_XiLanh_NongBan()
{
	if (target_BT_BanBong < Min_BT_BanBong )
		target_BT_BanBong = Min_BT_BanBong;
	if (target_BT_BanBong > Max_BT_BanBong)
		target_BT_BanBong = Max_BT_BanBong;

	if (abs(bientroBanBongValue - target_BT_BanBong) > 2)
	{
		speed_temp = (abs(bientroBanBongValue - target_BT_BanBong))*3;
		if (bientroBanBongValue > target_BT_BanBong)
		{
			Cylanh_back;
			if (speed_temp > 100) 
				speed_temp = 100; 	
...
void Kich_Xi_lanh_Ban_bong()
{
	if(TRIANGLE) target_BT_BanBong = 800;
	else if (X)  target_BT_BanBong  = 502;
}
...
//	}
	...
	void Bu_Tru_Luc()
	{
		if(L2 && O) {power += 1;vTaskDelay(2000);}
		if(L2 && X) {power -= 1;vTaskDelay(2000);}
	}
...
////Hieu
//void lay_khoan_cach_td(int lkc)
//{

//	if(state_nang == 1){
//	if(lazeTraiValue<=625 ){
//	gt_td=lazeTraiValue;
//	}
//	if (gt_td != prev_gt_td) {
//      UpdateArray(gt_td);  
...
//	
//}
//void UpdateArray(int new_value) {
//    
//    if (index_ < 100) {
//        data[index_] = new_value;
//        index_++;
//    }
//}

void LayGTLazeTudong()
{
		if (!R1 && X  && (target_BT_Xoay < Max_BT_Xoay))
	{
			bit_xoayTay = 1; 	
			Mor_CamRotate = 45;
			Mor_CamRotate_next;
			target_BT_Xoay = BienTroXoayValue;
				...
				if(lazeTraiValue < 624 && lay_khoang_cach_a == 0)
				{
					KhoangCachA = lazeTraiValue;
					lay_khoang_cach_a =1;
				}
				if(lazeTraiValue < 624 )
				{
					KhoangCachB = lazeTraiValue;	
		}	
	}
	//else bit_xoayTay = 0;
}
void Reset_khoangcach()
{
	lay_khoang_cach_a = 0;
	KhoangCachA = 0;
	KhoangCachB = 0;
...
void testXilanhNong(vu8 speed)
{
	if(TRIANGLE) 			Cylanh = speed , Cylanh_next;
	else if (X)				Cylanh = speed , Cylanh_back;
	else 							Cylanh = 0;
