...
int state_nang_3 = 0;
int test_bit = 0;
int Tay_Kep_Home = 0 , Ha_Tay_Kep = 0 ,Nang_Tay_Kep_KFS = 0 ;
...
void Auto_Nang_Ha_Tay_Kep()
{
	if (Tay_Kep_Home == 1)
	{
		testxung =  Nang_tay_tu_tu(0, testxung, 10.0f, 4200, 0.1f);
	}
	else if (Ha_Tay_Kep == 1)
	{
		testxung =  Nang_tay_tu_tu(25000, testxung, 10.0f, 4200, 0.1f);
	}
	else if (Nang_Tay_Kep_KFS == 1)
	{
		testxung =  Nang_tay_tu_tu(18000, testxung, 10.0f, 4200, 0.1f);
	}
...
void Tu_Dong_Lay_KFS(void)
{
	int i;
	DC_TAY_KEP_RA , DC_TAY_KEP = 40;
	for(i=0;i<100;i++)while(CB_HANH_TRINH_TAY_KEP_RA == 1) {if(BreakHam()) test_bit = 5; break; vTaskDelay(1);} //Chay ra Ô 1
	DC_TAY_KEP = 0; // Chu Y Reset Bit
	Ha_Tay_Kep = 1;
	Auto_Nang_Ha_Tay_Kep();
	for(i=0;i<100;i++)while(CB_TAY_KEP_KFS == 1) {if(BreakHam()) test_bit = 5; break; vTaskDelay(1);}
	Ha_Tay_Kep = 0; //Reset
	XI_LANH_KEP_ON;
	Nang_Tay_Kep_KFS = 1; // VTRI_THU_KFS
	DC_TAY_KEP_VAO , DC_TAY_KEP = 40;
	for(i=0;i<100;i++)while(CB_HANH_TRINH_TAY_KEP_VAO == 1) {if(BreakHam()) test_bit = 5; break; vTaskDelay(1);}
	XI_LANH_KEP_OFF;
	XI_LANH_GIU_KFS_TRUOC_ON;
	Nang_Tay_Kep_KFS = 0; //Reset
	Tay_Kep_Home = 1; // Ve Home
	DC_TAY_KEP_RA , DC_TAY_KEP = 25;
	vTaskDelay(10000);
