...
static void Task_Control_1(void *pvParameters) {   
		...
		for(;;) {
			...
			Test_Tay_Kep();
			Test_Lay_VK();
//			if(TAM_GIAC && CB_HANH_TRINH_TAY_KEP_RA == 1) Tu_Dong_KFS();
//			Bit_tay_ra_vao();
//			Co_Cau_Tay();
			Nang_Ha_Tay_Kep();
			//if(OPTIONS) Bit_OPTIONS == 1;
			if(OPTIONS)Test_Day_KFS();
...
//						XI_LANH_DAY_KFS_VAO;
//						XI_LANH_GIU_KFS_SAU_OFF;
//						XI_LANH_GIU_KFS_TRUOC_OFF;
						XI_LANH_KEP_OFF;
//						XI_LANH_TAY_KEP_XUONG;
					}
		...
		//	if(PICK_LANE == 1) Chay_Lane_1();
				//	Test_CB_Bang_tay();
				//if(O)FlagCB_DEMO_PRESS_O = 1;
				//	if(O)TuDongCanGiua(96 , 103);
			//else FlagCB_DEMO_PRESS_O = 0;
				//if(VUONG)MAIN_CONTROL(-5400,0,10,2,20);
			
//			if (PICK_LANE == 1 || PICK_LANE == 2 || PICK_LANE == 3)
//        {
//            Start_Auto_Lane_Logic();
//             robot_state = 0; 
//        }
...
//			if(VUONG) Test_Nang_Chan();
//			if(VUONG) CB_Truoc = 100;
//			if(O) UP_All_LEG();
//			
			//if(OPTIONS) CONTROL_TOC_DO(0,3,0) , robot_state = 0 ,PICK_LANE = 0 ,PICK_LANE = 0 , PICK_LANE = 0  ;
//			//if(PICK_LANE == 2)Start_auto_Cell_2();
//			//if(PICK_LANE == 1 || PICK_LANE == 2 || PICK_LANE == 3)Check_VT1();
//			if(PICK_LANE == 2)Test_Run_auto_noleg();
//		if(TOUCHPAD)
//		{	
//			flag_ha=0; 
//			flag_nang=0;
...
//				nang_chan_sau = nang_chan_tu_tu(30200, nang_chan_trai, 10, 200, 0.1f);
//				DC_nang_phai(nang_chan_phai);
//        DC_nang_trai(nang_chan_trai);
//        DC_nang_sau (nang_chan_sau);
//		}
