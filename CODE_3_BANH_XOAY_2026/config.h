...
#define	DIA_CHI_START7   			DATA_SPEED[30]=255    // �ong co n�ng -- cau H
...
void Config_out_mode(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOD|RCC_AHB1Periph_GPIOB|RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOC, ENABLE);
	///////khai b�o nhung chan hien thi tren main C15,D7
  ...
  GPIO_Init(GPIOC, &GPIO_InitStructure); 
	///////khai b�o nhung chan OUTPUT A0,A1,A8,A12,B0,B6,B7,B8,B9,B10,B11,B12,B13,B14,B15
...
void Config_in_mode(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB | RCC_AHB1Periph_GPIOC | RCC_AHB1Periph_GPIOE  , ENABLE);
	///////khai b�o N�T NHAN XANH DO tren main 
...
//	
  ...
  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	///////khai b�o nhung chan IN tren main 
...
void Config_ADC1_DMA(void)
{
  ...
  RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1, ENABLE);
   ...
   DMA_ITConfig(DMA2_Stream0, DMA_IT_TC, ENABLE);  // Ngắt khi truyền xong
   //DMA_ITConfig(DMA2_Stream0, DMA_IT_HT, ENABLE);  // (tùy chọn) ngắt giữa chừng
  ...
  ADC_TempSensorVrefintCmd(ENABLE); // Bật đo Vrefint để tự kiểm tra ổn định nguồn
...
{		
    ...
    DMA_InitStructure.DMA_Channel = DMA_Channel_4;
    DMA_InitStructure.DMA_PeripheralBaseAddr = (uint32_t)&USART3->DR;
    DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)GP_BTN;  // buffer nhận
    DMA_InitStructure.DMA_DIR = DMA_DIR_PeripheralToMemory;
    ...
    DMA_InitStructure.DMA_PeripheralDataSize = DMA_PeripheralDataSize_Byte;
    DMA_InitStructure.DMA_MemoryDataSize = DMA_MemoryDataSize_Byte;
    DMA_InitStructure.DMA_Mode = DMA_Mode_Circular;       // ✅ để circular nếu Timer trigger
    DMA_InitStructure.DMA_Priority = DMA_Priority_High;
    DMA_InitStructure.DMA_FIFOMode = DMA_FIFOMode_Disable;
    DMA_Init(DMA1_Stream3, &DMA_InitStructure);
...
//        // Giả sử mảng dữ liệu bạn dùng tên là DATA_RC[]
//        // và độ dài dữ liệu = 10 byte (tùy bạn điều chỉnh)
//        RC_ID=id_rc;
//        RC_1(RC1);		         // ((DATA_RC[7]=A&0xFF)|(DATA_RC[6]=(A>>8)&0xFF)) // tốc độ
...
void JOHO_CreatePacket(uint8_t id, uint16_t pos, uint16_t time, uint8_t *buf)
{
    ...
    buf[1] = 0xFF;
    buf[2] = id;
    buf[3] = 0x07;      // LEN (nhu b?n dang d�ng)
    buf[4] = 0x03;      // CMD
    buf[5] = 0x2A;      // ADDR
    buf[6] = (pos >> 8) & 0xFF;
    buf[7] = pos & 0xFF;
	...
	void TIMER_RC(void)
	{
													 if (dma_busy == 0) // Chỉ gửi khi DMA rảnh
							 {
...
//    // --- G�i 1: ghi byte th?p v�o d?a ch? 0x10 ---
//    buf[0] = 0xFF;
//    buf[1] = 0xFF;
//    buf[2] = id;
//    buf[3] = 0x04;
//    buf[4] = 0x03;
//    buf[5] = 0x10;   // �?a ch? th?p
//    buf[6] = low;
//    uint8_t sum1 = 0;
//    for (uint8_t i = 2; i < 7; i++) sum1 += buf[i];
//    buf[7] = ~sum1;

//    // --- G�i 2: ghi byte cao v�o d?a ch? 0x11 ---
//    buf[8]  = 0xFF;
//    buf[9]  = 0xFF;
//    buf[10] = id;
//    buf[11] = 0x04;
//    buf[12] = 0x03;
//    buf[13] = 0x11;  // �?a ch? cao
//    buf[14] = high;
//    uint8_t sum2 = 0;
//    for (uint8_t i = 10; i < 15; i++) sum2 += buf[i];
//    buf[15] = ~sum2;
...
//						break;
				...
				case 10:
						HMI_DMI("_TOC_DO_BT: ", _TOC_DO_BT , 10);	 																
						break;
									
				case 11:
						HMI_DMI("bien_xuong_200: ", TOC_DO, 11);						
						break;
                ...
                GPIO_ReadOutputDataBit(GPIOB, GPIO_Pin_7)
						...
						case 19:
						HMI_DMI("HUONG_GAME_CT: ", HUONG_GAME_CT, 19);										
						break;
				case 20:
				...
				case 27:
				   	HMI_DMI("ENCODER_BANH_TRAI: ", ENCODER_BANH_TRAI, 27);	
						break;
...
//								RX_USART6[1],
						...
						break;
				case 37:
					//	HMI_DMI("IMU_X: ", IMU_X, 37);	
						break;
									
				case 38: 
					//	HMI_DMI("IMU_Y: ", IMU_Y, 38);					
						break;
									
				case 39:
					//	HMI_DMI("_ADC1_Filtered[10]: ",_ADC1_Filtered[10] , 39);					
						break;
									
				case 40:
					//	HMI_DMI("_ADC2_Filtered[10]: ", _ADC1_Filtered[9], 40);										
						break;		
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx H� DIEU HANH THOI GIAN THUC xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
/*	
	h�m khoi tao chay song song bang bo thu vien FREE RT OS
	khi su dung h�m n�y th� 3 h�m ngat sau khong c�n su dung b�nh thuong duoc nua\
...
void DAI_RTOS_V1(void)	
  {	
	...
	//led_task									//	la ten cua tac vu duoc tro den h�m chua ten t�c vu do
